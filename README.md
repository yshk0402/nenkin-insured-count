...
このCLIは、日本年金機構の「厚生年金保険・健康保険 適用事業所検索システム」と、国税庁法人番号公表サイトの公開情報を利用します。

法人番号の解決機能について、このCLIの出力を利用したサービスや資料を公開する場合は、国税庁法人番号公表サイトの利用条件に従い、取得元を明示してください。
...
国税庁法人番号システムのWeb-API機能を利用するサービスとして公開する場合は、国税庁が案内している次の表示も、利用者が参照できる場所に記載してください。

```text
このサービスは、国税庁法人番号システムのWeb-API機能を利用して取得した情報をもとに作成しているが、サービスの内容は国税庁によって保証されたものではない
```

なお、法人番号システムWeb-APIの利用にはアプリケーションIDが必要です。このCLIの `resolve` は、現時点ではアプリケーションIDを使う公式Web-APIではなく、国税庁法人番号公表サイトの検索画面から公開情報を取得します。将来Web-APIモードを追加する場合は、利用規約への同意、アプリケーションIDの取得、上記表示の維持を前提にしてください。
